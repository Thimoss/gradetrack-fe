...
import {
  calculateMtrParameterScore,
  formatMtrScore,
  mtrAssessmentParameters,
} from "./mtr-assessment-data";

type SelectedMtrAssessment = Record<string, number>;

type MtrAssessmentSectionProps = {
...
export function MtrAssessmentSection({
  onSummaryChange,
}: MtrAssessmentSectionProps) {
  const [selectedAssessment, setSelectedAssessment] =
    useState<SelectedMtrAssessment>({});

  const summary = useMemo(() => {
    return mtrAssessmentParameters.reduce(
      (current, parameter) => {
        const selectedValue = selectedAssessment[parameter.code];
        const score = calculateMtrParameterScore(
          parameter.weight,
          selectedValue,
        );
    ...
    <GradingFormSection
      ...
      headerSlot={
        <span className="rounded-full bg-slate-900 px-3 py-1 text-xs font-bold text-white">
          Total {formatMtrScore(summary.totalScore)}
        </span>
      }
      bodyClassName="p-0"
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[940px] border-collapse text-sm text-neutral-950">
          <thead>
            <tr className="bg-slate-100">
              <th className="w-10 border border-zinc-200 px-2 py-3 text-center font-bold text-slate-700">
                No
              </th>
              <th className="w-60 border border-zinc-200 px-3 py-3 text-center font-bold text-slate-700">
                Parameter
              </th>
              <th
          ...
          <tbody>
            {mtrAssessmentParameters.map((parameter) => {
              const selectedValue = selectedAssessment[parameter.code];
              const score = calculateMtrParameterScore(
                parameter.weight,
                selectedValue,
              ...
              return parameter.options.map((option, optionIndex) => {
                ...
                const selectionId = `mtr-${parameter.code}-${option.value}`;
            ...
            <tr>
              ...
              <td className="border border-zinc-200 bg-slate-50 px-3 py-3 text-center font-bold">
                {formatMtrScore(summary.totalScore)}
              </td>
            </tr>
          </tbody>
        </table>
