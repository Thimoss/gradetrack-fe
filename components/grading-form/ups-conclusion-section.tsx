...
import { getUpsNextInspectionInterval } from "./ups-conclusion-data";

type UpsConclusionSectionProps = {
  totalScore: number;
  improvementParameters: string[];
};

export function UpsConclusionSection({
  totalScore,
  improvementParameters,
}: UpsConclusionSectionProps) {
  const hasAssessmentInput = totalScore > 0 || improvementParameters.length > 0;
  const nextInspectionInterval = getUpsNextInspectionInterval(totalScore);
  ...
  return (
    <GradingFormSection title="Kesimpulan">
      <div className="grid grid-cols-[minmax(220px,0.8fr)_16px_minmax(320px,1fr)] gap-x-6 gap-y-6 text-sm text-[#232122] max-md:grid-cols-1">
        ...
        <label
          className="py-2 font-medium text-neutral-700"
          htmlFor="ups-major-condition-summary"
        >
        ...
        <label
          className="py-2 font-medium text-neutral-700"
          htmlFor="ups-improvement-parameters"
        >
        ...
        <label className="py-2 font-medium text-neutral-700" htmlFor="ups-notes">
          Catatan
        </label>
        <span className="py-2 text-center text-neutral-400 max-md:hidden">:</span>
        <textarea
