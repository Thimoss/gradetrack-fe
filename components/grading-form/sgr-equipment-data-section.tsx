...
import {
  buildSgrEquipmentRows,
  type SgrEquipmentData,
} from "./sgr-equipment-data";

type SgrEquipmentDataSectionProps = {
  equipment?: SgrEquipmentData | null;
  isLoading?: boolean;
};

export function SgrEquipmentDataSection({
  equipment,
  isLoading = false,
}: SgrEquipmentDataSectionProps) {
  if (isLoading) {
    return (
  ...
  const rows = buildSgrEquipmentRows(equipment);
