...
import {
  calculatePipParameterScore,
  formatPipScore,
  pipAssessmentParameters,
} from "./pip-assessment-data";

type SelectedPipAssessment = Record<string, number>;

type PipAssessmentSectionProps = {
...
export function PipAssessmentSection({
  onSummaryChange,
}: PipAssessmentSectionProps) {
  const [selectedAssessment, setSelectedAssessment] =
    useState<SelectedPipAssessment>({});

  const summary = useMemo(() => {
    return pipAssessmentParameters.reduce(
      (current, parameter) => {
        const selectedValue = selectedAssessment[parameter.code];
        const score = calculatePipParameterScore(
          parameter.weight,
          selectedValue,
        );
    ...
    <GradingFormSection
      ...
      headerSlot={
        <span className="rounded-full bg-slate-900 px-3 py-1 text-xs font-bold text-white">
          Total {formatPipScore(summary.totalScore)}
        </span>
      }
      bodyClassName="p-0"
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1020px] border-collapse text-sm text-neutral-950">
          <thead>
          ...
          <tbody>
            {pipAssessmentParameters.map((parameter) => {
              const selectedValue = selectedAssessment[parameter.code];
              const score = calculatePipParameterScore(
                parameter.weight,
                selectedValue,
              ...
              return parameter.options.map((option, optionIndex) => {
                ...
                const selectionId = `pip-${parameter.code}-${option.value}`;
                ...
                return (
                  <tr key={`${parameter.code}-${option.value}`}>
                    ...
                    >
                      <label
                        className="flex h-full min-h-9 w-full cursor-pointer items-center justify-center px-2"
                        htmlFor={selectionId}
                      >
                        <input
                          checked={isSelected}
                    ...
                    <td className="border border-zinc-200 px-4 py-1.5 leading-6">
                      {option.description}
                    </td>
                    <td className="border border-zinc-200 px-3 py-1.5 text-center font-semibold">
                      {option.value}
                    </td>
                    {optionIndex === 0 ? (
                      <td
            ...
            <tr>
              ...
              <td className="border border-zinc-200 bg-slate-50 px-3 py-3 text-center font-bold">
                {formatPipScore(summary.totalScore)}
              </td>
            </tr>
          </tbody>
        </table>
