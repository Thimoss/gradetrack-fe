...
import {
  calculatePmpParameterScore,
  formatPmpScore,
  pmpAssessmentParameters,
} from "./pmp-assessment-data";

type SelectedPmpAssessment = Record<string, number>;

type PmpAssessmentSectionProps = {
...
export function PmpAssessmentSection({
  onSummaryChange,
}: PmpAssessmentSectionProps) {
  const [selectedAssessment, setSelectedAssessment] =
    useState<SelectedPmpAssessment>({});

  const summary = useMemo(() => {
    return pmpAssessmentParameters.reduce(
      (current, parameter) => {
        const selectedValue = selectedAssessment[parameter.code];
        const score = calculatePmpParameterScore(
          parameter.weight,
          selectedValue,
        );
    ...
    <GradingFormSection
      ...
      headerSlot={
        <span className="rounded-full bg-[#036CB6] px-3 py-1 text-xs font-bold text-white">
          Total {formatPmpScore(summary.totalScore)}
        </span>
      }
      bodyClassName="p-0"
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1040px] border-collapse text-sm text-[#232122]">
          <thead>
          ...
          <tbody>
            {pmpAssessmentParameters.map((parameter) => {
              const selectedValue = selectedAssessment[parameter.code];
              const score = calculatePmpParameterScore(
                parameter.weight,
                selectedValue,
              ...
              return parameter.options.map((option, optionIndex) => {
                ...
                const selectionId = `pmp-${parameter.code}-${option.value}-${optionIndex}`;

                return (
                  <tr key={`${parameter.code}-${option.value}-${optionIndex}`}>
                    {optionIndex === 0 ? (
                      <>
                        <td
                    ...
                    >
                      <label
                        className="flex h-full min-h-10 w-full cursor-pointer items-center justify-center px-2"
                        htmlFor={selectionId}
                      >
                        <input
                          checked={isSelected}
            ...
            <tr>
              ...
              <td className="border border-zinc-200 bg-slate-50 px-3 py-3 text-center font-bold">
                {formatPmpScore(summary.totalScore)}
              </td>
            </tr>
          </tbody>
        </table>
