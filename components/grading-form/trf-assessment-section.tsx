...
import {
  calculateTrfParameterScore,
  formatTrfScore,
  trfAssessmentParameters,
} from "./trf-assessment-data";

type SelectedTrfAssessment = Record<string, number>;

type TrfAssessmentSectionProps = {
...
export function TrfAssessmentSection({
  onSummaryChange,
}: TrfAssessmentSectionProps) {
  const [selectedAssessment, setSelectedAssessment] =
    useState<SelectedTrfAssessment>({});

  const summary = useMemo(() => {
    return trfAssessmentParameters.reduce(
      (current, parameter) => {
        const selectedValue = selectedAssessment[parameter.code];
        const score = calculateTrfParameterScore(
          parameter.weight,
          selectedValue,
        );
    ...
    <GradingFormSection
      ...
      headerSlot={
        <span className="rounded-full bg-[#036CB6] px-3 py-1 text-xs font-bold text-white">
          Total {formatTrfScore(summary.totalScore)}
        </span>
      }
      bodyClassName="p-0"
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1040px] border-collapse text-sm text-[#232122]">
          ...
          <tbody>
            {trfAssessmentParameters.map((parameter) => {
              const selectedValue = selectedAssessment[parameter.code];
              const score = calculateTrfParameterScore(
                parameter.weight,
                selectedValue,
              ...
              return parameter.options.map((option, optionIndex) => {
                ...
                const selectionId = `trf-${parameter.code}-${option.value}`;
            ...
            <tr>
              ...
              <td className="border border-zinc-200 bg-slate-50 px-3 py-3 text-center font-bold">
                {formatTrfScore(summary.totalScore)}
              </td>
            </tr>
          </tbody>
        </table>
