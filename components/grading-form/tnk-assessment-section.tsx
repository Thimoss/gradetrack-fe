...
import {
  calculateTnkParameterScore,
  formatTnkScore,
  tnkAssessmentParameters,
} from "./tnk-assessment-data";

type SelectedTnkAssessment = Record<string, number>;

type TnkAssessmentSectionProps = {
...
export function TnkAssessmentSection({
  onSummaryChange,
}: TnkAssessmentSectionProps) {
  const [selectedAssessment, setSelectedAssessment] =
    useState<SelectedTnkAssessment>({});

  const summary = useMemo(() => {
    return tnkAssessmentParameters.reduce(
      (current, parameter) => {
        const selectedValue = selectedAssessment[parameter.code];
        const score = calculateTnkParameterScore(
          parameter.weight,
          selectedValue,
        );
    ...
    <GradingFormSection
      ...
      headerSlot={
        <span className="rounded-full bg-[#036CB6] px-3 py-1 text-xs font-bold text-white">
          Total {formatTnkScore(summary.totalScore)}
        </span>
      }
      bodyClassName="p-0"
    >
      <div className="overflow-x-auto">
        <table className="w-full min-w-[1040px] border-collapse text-sm text-[#232122]">
          ...
          <tbody>
            {tnkAssessmentParameters.map((parameter) => {
              const selectedValue = selectedAssessment[parameter.code];
              const score = calculateTnkParameterScore(
                parameter.weight,
                selectedValue,
              ...
              return parameter.options.map((option, optionIndex) => {
                ...
                const selectionId = `tnk-${parameter.code}-${option.value}`;
                    ...
                    <td
                      className={`w-24 border border-zinc-200 p-0 transition ${
                        isSelected
                          ...
                          : "bg-white text-slate-700 hover:bg-[#E6F1FA]"
                      }`}
                    >
                      <label
            ...
            <tr>
              ...
              <td className="border border-zinc-200 bg-slate-50 px-3 py-3 text-center font-bold">
                {formatTnkScore(summary.totalScore)}
              </td>
            </tr>
          </tbody>
        </table>
